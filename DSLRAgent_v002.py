## @package DSLRAgent
#	This agent provides interface to control DSLR camera from a remote client
#	It supports one user and allows taking photo and retreiving the image file.
#
#----------------------------------------------------------------------------------
#17.09.2017	Revision history started
#		Start AGNT_ABORTSHOT_REQ implementation
#
#----------------------------------------------------------------------------------
...
class Shot:
"""@brief Shot object.
The shot objectt is created during DSLR exposing a shot.
It is needed to check if exposuring was done in a correct way.
@param filename - Upon shot is finished it contains the name of created file
"""
	...
	def __init__(self, isoVal, expVal):
		"""Constructor takes ISO and exposure"""
		self.iso = isoVal
...
"""The thread is started to take a long exposure photo.
   It initiates GPHOTO subprocess and checks the output in a loop while DSLR is exposing.
   The thread sends messages to the client socket (AGNT_EXPOSING_IND) so client is aware about the progress.
"""
	...
	def run(self):
		...
		#result = GPHOTO2_MakeshotReq(self.iso, self.exposure)
		...
		while (prcss.poll() == None) :
			#Check if we need to leave the thread
			if (True == exitFlag) :
				ConnSend("AGNT_MAKESHOT_CONF:ABORTED#")
				exit
...
def Process_CONNECT_REQ():
"""Function handles connect request from a client.
   It checks that there is some DSLR to work with and includes its name in confirmation.
   Otherwise the FAILED result is included in confirmation
"""
	CameraName = GetCameraInfo()
	if (CameraName == "NONE"):
		return "AGNT_CONNECT_CONF:FAILED#"
	else:
		return "AGNT_CONNECT_CONF:SUCCESS:" + CameraName + "#"
...
def DUMMY_StartTransferThread(filename):
	...
	pureFileName = None
	try:
		print "Creating transfSkt ..." + str(transPort)
...
#TBD: not sure if this will work.
# Check how multithread works in Python (if thread class allows changing variables)
def Process_ABORTSHOT_REQ():
	if(SHOOTING_EXPOSING == shootingThread.GetState()):
		shootingTrhead.SetExit();
	return None
...
def DispatchCommand(commandStr, scktConn):
	...
	if(AGNT_CONNECT_REQ == msgName):   return Process_CONNECT_REQ() #DUMMY_ConnectReq() #Process_CONNECT_REQ()
	...
	elif(AGNT_ABORTSHOT_REQ == msgName):   return Process_ABORTSHOT_REQ(parts[1])
